mapboxgl.accessToken =
  "pk.eyJ1IjoicGV0ZXJxbGl1IiwiYSI6ImNqdHJqdG92OTBkNTg0M3BsNDY0d3NudWcifQ.0z4ov_viFE-yBMSijQpOhQ";
...
scale.on("load", function () {
  scale.addSource("square", { type: "geojson", data: tinySquare }).addLayer({
    id: "extrusions",
    type: "fill",
    source: "square",
    paint: {
  ...
  features: [
    {
      ...
      geometry: {
        type: "Point",
        coordinates: [-122.4232292175293, 37.784282779035216]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-118.28636169433594, 34.04412546508576]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-122.33327865600585, 47.60651025683697]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-90.10042190551758, 29.946159058399612]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-87.6240348815918, 41.87531293759582]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-75.1626205444336, 39.95370120254379]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-73.99772644042969, 40.72228267283148]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-84.39079284667969, 33.74910736130734]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-122.67179489135742, 45.522104713562825]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-104.99565124511719, 39.74428621972816]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-93.26637268066406, 44.969656023708175]
      }
    },
    {
      type: "Feature",
      ...
      geometry: {
        type: "Point",
        coordinates: [-80.19624710083008, 25.773846629676616]
      }
    }
  ]
};
...
greatPlaces.features.forEach(function (city, index) {
  ...
  document
    .getElementById("city" + index)
    .setAttribute("onclick", "jumpToCity(" + index + ")");
});
...
map.addControl(
  new mapboxgl.Geocoder({
    ...
    placeholder: "Explore any US city...",
    country: "us"
  })
);
...
map.on("load", function () {
  ...
  map
    .on("rotate", function () {
      scale.setBearing(map.getBearing());
    })
    .on("zoom", function () {
      var multiplier = map.getZoom() - 12;
      document.querySelector("#people").innerHTML = Math.ceil(
        4000 * Math.pow(1 / 8, multiplier)
      );
    })
    .on("move", function () {
      scale.setPitch(map.getPitch() * 0.7); //fudge factor to get it to look right on the lower-left corner
    });
  ...
  map
    .on("mousemove", function (e) {
      if (inspector === "block") {
        ...
        var showTooltip =
          ft && ft.properties.p > 0 && e.originalEvent.which === 0;

        if (showTooltip) {
          mapObj.classList = "inspector";

          tooltip.style.transform =
            "translateX(" + e.point.x + "px) translateY(" + e.point.y + "px)";
          ...
          if (ft.properties.id !== currentBlock) {
            ...
            var queryURL =
              ...
              coord +
              ".json?access_token=pk.eyJ1IjoicGV0ZXJxbGl1IiwiYSI6ImNqdHJqdG92OTBkNTg0M3BsNDY0d3NudWcifQ.0z4ov_viFE-yBMSijQpOhQ";
            mapboxgl.util.getJSON(queryURL, function (err, resp) {
              currentBlock = ft.properties.id;
              var address =
                resp.features[0].address + " " + resp.features[0].text;
              document.querySelector("#address").innerHTML =
                "Near " + address.replace("undefined ", "");
            });
          }
        } else {
...
function tilt(eh) {
  var state = !eh
    ? { pitch: 0, klass: [""] }
    : { pitch: 50, klass: ["tilted"] };

  document.querySelector("#legends").className =
    "pin-bottomright scale " + state.klass[0];
  map.easeTo({ pitch: state.pitch }).setClasses(state.klass);
...
function setScale(max) {
  ...
  scale.setPaintProperty("extrusions", "fill-extrude-height", {
    stops: [
      [0, 10],
      [max, 20000]
    ],
  ...
  map
    .setPaintProperty("fills", "fill-color", {
      stops: [
        [0, "#160e23"],
        [max * 0.02, "#00617f"],
        [max * 0.1, "#55e9ff"]
    ...
    .setPaintProperty("highlighted_extrusion", "fill-extrude-height", {
      stops: [
        [0, 10],
        [max, 20000]
      ],
...
function setInspector(mode) {
  ...
  if (mode === "radius")
    document.querySelector("#address").innerHTML = "Within 500 meters of here";
}
...
function drawRadiusCircle() {
  ...
  var pixelRadius =
    map.project(edgeX.geometry.coordinates).x - map.project(lnglat).x;
